...
received_msg = []
alice_val = 100
shares = help.split_num_to_shares(alice_val)
message_to_bob = str(shares[1])
message_to_charlie = str(shares[2])
received_msg.append(str(shares[0]))
# Function to start the server
def start_alice_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_sock:
        server_sock.bind(('0.0.0.0', 8444))
        server_sock.listen(5)

        print("Alice server is listening on port 8444...")
...
if __name__ == '__main__':
    ...
    server_thread = threading.Thread(target=start_alice_server)
    server_thread.start()
