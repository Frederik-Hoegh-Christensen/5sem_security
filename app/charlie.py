...
def server():
    ...
    print("Charlies server is listening on port 8082")
    while True:
...
def main():
    ...
    result = help.SplitToShare(message, charlie_input)
    ...
    # Launching client with bobs server as port, in another thread
    client_thread = threading.Thread(
        target=client(str_convert[2], 8081))
    client_thread.start()
    ...
    charlie_sum = str(ShareSumToSend)
    print("Charlies sum to be send to hospital:", charlie_sum)

    # Launching client with hospitals server as port, in a final thread
    client_thread = threading.Thread(
        target=client(charlie_sum, 8083))
    client_thread.start()
